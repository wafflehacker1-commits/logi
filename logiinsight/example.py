...
def draw_face_bbox(frame, face, frame_w, frame_h, sensor_w=3840, sensor_h=2160):
    ...
    if len(bbox) < 4:
        return
    bx, by, bw, bh = bbox
    sx, sy = frame_w / sensor_w, frame_h / sensor_h
...
def main():
    ...
    if args.device:
        dev_index = int(args.device.replace("/dev/video", ""))
    else:
        dev_index = find_uvc_device()
        if dev_index < 0:
            print("  ERROR: No UVC device found. Check USB connection.")
            cam.disconnect()
            sys.exit(1)
